...
def main():
	...
	if not args.regenerate and os.path.exists(corpus_path):
		...
		if args.dataset == "MovieLens_1M":
    		# 进行采样，选择 20% 的数据
			data = corpus.data_df['train']
			sampled_data = data.sample(frac=0.2, random_state=42)
    		# 将采样后的数据重新赋值给原字典中的对应数据集
			corpus.data_df['train'] = sampled_data
	else:
...
if __name__ == '__main__':
	...
	# 修改这里的模型名称解析逻辑
	if init_args.model_name == 'SHTModel':
		model_name = SHTModel  # 直接使用类名
	else:
